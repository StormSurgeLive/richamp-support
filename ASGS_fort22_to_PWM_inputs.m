...
B2(1:end,5)=0;
B2(:,6)=datenum(B2(:,1),B2(:,2),B2(:,3),B2(:,4),B2(:,5),B2(:,5));
%%
[C,ia,ic] = unique(B2(:,6));
ys=num2str(B2(1,1)); 
...
At(:,7)=datenum(At(:,1),At(:,2),At(:,3),At(:,4),At(:,5),At(:,6));
tstr=datestr(At(:,7));
Rdeg=km2deg(Ad(:,7));
R34=km2deg(Ad(:,8));
%% Radius of max winds
...
% Rmx_c1=0.00005086*(dP.^2);
% Rmx_c2=0.0394899*(LL(:,1));
% Rmx_c3=2.636-Rmx_c1+Rmx_c2;
% Rmxc=exp(Rmx_c3);
%% radius of closure (pressure)
for i=1:length(Ad(:,7))
    if(Ad(i,7)==99)
     Ra(i)=fix(Rmxc(i)*20);    % if calculate
    else
     Ra(i)=Ad(i,7);
    end
end
...
for n=1:sz(1)
  ...
  %%  
    strz='0000';
     ...
     if(Ad(n,8)==0);Ad(n,8)=-999;end; str14a=pad(num2str(Ad(n,8)),4,'left','0');  %34kt NE 
     if(Ad(n,9)==0);Ad(n,9)=-999;end; str14b=pad(num2str(Ad(n,9)),4,'left','0');  %34kt SE
     if(Ad(n,10)==0);Ad(n,10)=-999;end; str14c=pad(num2str(Ad(n,10)),4,'left','0');  %34kt SW
     if(Ad(n,11)==0);Ad(n,11)=-999;end; str14d=pad(num2str(Ad(n,11)),4,'left','0');  %34kt NW
   strL='D';
    if(Ad(n,12)==0);Ad(n,12)=-999;end; str15a=pad(num2str(Ad(n,12)),4,'left','0');  %50kt NE
    if(Ad(n,13)==0);Ad(n,13)=-999;end; str15b=pad(num2str(Ad(n,13)),4,'left','0');  %50kt SE
    if(Ad(n,14)==0);Ad(n,14)=-999;end; str15c=pad(num2str(Ad(n,14)),4,'left','0');  %50kt SW
    if(Ad(n,15)==0);Ad(n,15)=-999;end; str15d=pad(num2str(Ad(n,15)),4,'left','0');  %50kt NW
